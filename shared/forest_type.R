...
map_to_regional = function(ftype_ras_df, region){
  ...
  new_col = "reg_d" #add the new column to ftype_ras_df
  
  for(row in 1:nrow(ftype_ras_df)){# length :xi yi county_val value #nrow(ftype_ras_df)
    val = ftype_ras_df$value[row] #0-106 View(ftype_ras_df)
  
    if (is.na(val)){
      ftype = "other"
    } else{
      ftype = ftype_dbf[ftype_dbf$Value==val,]$REGIONAL_D #based on original data
    }
...
get_ftype_raster = function(){
  ...
  ns =  raster(ftype_files[1])#/gpfs/data1/cmongp/ujjwal/cec/Carmen_data/NSierra.tif 
...
get_south_ftype_raster = function(){
  ...
  ftype_files = list.files(carmen_data,pattern=glob2rx("*.tif"),full.names = T)
  ss =  raster(ftype_files[1])#/gpfs/data1/cmongp/ujjwal/cec/Carmen_data/New/SouthSierra_new.tif 
...
get_ftype_dbf = function(region){
  ...
  if(region =="north"){
    ...
    dbf_file = read.dbf(dbf_files[10], as.is=TRUE)#"/gpfs/data1/cmongp/ujjwal/cec/Carmen_data/NSierra.tif.vat.dbf" dbf_files[7] is NorthInterior1.tif.vat.dbf 
    dbf_file = dbf_file[,c(1,3)]
  } else {
    carmen_data_new = paste0(carmen_data,"New")
    dbf_files = list.files(carmen_data_new,pattern=glob2rx("*.dbf"),full.names = T)#"gpfs/data1/cmongp/ujjwal/cec/Carmen_data/New/SSierra_attirbutes.dbf"
    dbf_file = read.dbf(dbf_files[1], as.is=TRUE)
    names(dbf_file)[1] = "Value"
    View(dbf_file)
    dbf_file = dbf_file[,c(1,2)] #the output is two columns for value and Regional
  ...
  for(row in 1:nrow(dbf_file)){
    ...
    dbf_file$REGIONAL_D[row] = new_reg_d # return the regional_d with different type of forest,including pine, mixed conifer, and other.
  }
  #write.csv(dbf_file,"/gpfs/data1/cmongp/lansong/test/test_files/NorthInterior_reg.csv")
  View(dbf_file)
  return (dbf_file)
}
...
load_forest_type <- function(){
 ...
 north_shape = shapefile(paste0(carmen_data,ftype_files[1]))#EVMid_R05_NorthSierra.shp
